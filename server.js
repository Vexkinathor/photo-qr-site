...
app.post('/reveal/:id', (req, res) => {
  ...
  if (reachedLimit) {
    ...
    setTimeout(() => deletePhotoById(photo.id), 8000);
  }

  res.json({ ok: true, imgUrl: `/img/${photo.id}` });
});

// --- Page PUBLIQUE album : s'ouvre quand on scanne le QR code de l'album ---
app.get('/album/:id', (req, res) => {
  const album = readAlbums().find((a) => a.id === req.params.id);

  if (!album || album.expired || isTimedOut(album)) {
    return res.status(404).send('Album introuvable ou expire.');
  }
...
// --- Revelation d'un album : declenchee par le clic sur le bouton ---
app.post('/reveal-album/:id', (req, res) => {
  const albums = readAlbums();
  const album = albums.find((a) => a.id === req.params.id);
  ...
  const photos = readPhotos();
  const items = album.photoIds
    .map((id) => photos.find((p) => p.id === id))
    .filter(Boolean)
    .map((p) => ({ name: p.name, imgUrl: `/img/${p.id}` }));

  const reachedLimit = album.maxViews && album.viewCount >= album.maxViews;
  if (reachedLimit) {
    const current = readAlbums();
    const target = current.find((a) => a.id === album.id);
    if (target) {
      target.expired = true;
      writeAlbums(current);
    }
    setTimeout(() => deleteAlbumById(album.id), 8000);
  }

  res.json({ ok: true, items });
});

// --- Sert le fichier image reel, uniquement via l'id ---
